...
def part_lookup(part_number: str, requested_quantity: int = 1, customer_id: str = "0"):
    ...
    # Get product details (availability, specs, description)
    details_url = f"{API_BASE}/products/v4/search/{part_number}/productdetails"
    details = _make_request("GET", details_url, headers)

    # Get full pricing tiers
    pricing_url = f"{API_BASE}/products/v4/search/{part_number}/productpricing?requestedQuantity={requested_quantity}"
    pricing = _make_request("GET", pricing_url, headers)
    ...
    # Extract fields
    dk_pn = val(details.get("DigiKeyPartNumber"))
    mfr = val(details.get("Manufacturer", {}).get("Name"))
    mfr_pn = val(details.get("ManufacturerPartNumber"))
    desc = val(details.get("Description", {}).get("ProductDescription"))
    detailed_desc = val(details.get("Description", {}).get("DetailedDescription"))
    status = val(details.get("ProductStatus", {}).get("Status"))
    datasheet = details.get("DatasheetUrl")
    product_url = details.get("ProductUrl")
    qty_available = val(details.get("QuantityAvailable"))
    lead_weeks = val(details.get("ManufacturerLeadWeeks"))
    moq = val(details.get("MinimumOrderQuantity"))
    ...
    tiers = pricing.get("StandardPricing") or []
    if tiers:
        ...
        lines.append("|---------------:|------------|-------------|")
        for tier in tiers:
            bq = val(tier.get("BreakQuantity"))
            up = tier.get("UnitPrice")
            tp = tier.get("TotalPrice")
